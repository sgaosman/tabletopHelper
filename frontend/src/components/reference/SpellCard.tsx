    ...
    <div className="max-w-2xl bg-card border border-rule overflow-hidden" style={{ borderLeftWidth: '3px', borderLeftColor: accent }}>
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="grid grid-cols-2 gap-1.5 border-b border-rule-light pb-3">
          <p className="font-body text-[12px]"><span className="font-heading text-[9px] font-semibold tracking-[0.06em] uppercase text-faint">Casting Time</span><br/><span className="text-ink font-bold">{s.castingTime}</span></p>
          <p className="font-body text-[12px]"><span className="font-heading text-[9px] font-semibold tracking-[0.06em] uppercase text-faint">Range</span><br/><span className="text-ink font-bold">{s.rangeDistance}</span></p>
          <p className="font-body text-[12px]"><span className="font-heading text-[9px] font-semibold tracking-[0.06em] uppercase text-faint">Components</span><br/><span className="text-ink font-bold">{componentsStr}</span></p>
          <p className="font-body text-[12px]"><span className="font-heading text-[9px] font-semibold tracking-[0.06em] uppercase text-faint">Duration</span><br/><span className="text-ink font-bold">{s.duration}</span></p>
        </div>

        {s.concentration && (
          <span className="inline-block font-heading text-[9px] font-semibold tracking-[0.02em] px-1.5 py-0.5 text-muted border border-rule bg-page-alt">
      ...
      {s.classes && s.classes.length > 0 && (
        <div className="px-5 py-2 border-t border-rule">
          ...
          <span className="font-body text-[11px] font-semibold text-muted">{s.classes.join(', ')}</span>
        </div>
      )}
