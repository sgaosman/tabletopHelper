    ...
    <div className="max-w-2xl bg-card border border-rule overflow-hidden" style={{ borderLeftWidth: '3px', borderLeftColor: '#991B1B' }}>
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="border-b border-rule-light pb-3 space-y-1">
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Armour Class</span> {m.armourClass}{m.acType ? ` (${m.acType})` : ''}</p>
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Hit Points</span> {m.hitPoints}{m.hitDice ? ` (${m.hitDice})` : ''}</p>
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Speed</span> {speedStr}</p>
        </div>
          ...
          <div className="grid grid-cols-6 text-center gap-1">
            {(['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'] as const).map(ab => (
              <div key={ab} className="bg-page py-1.5 px-1">
                ...
                <div className="font-body text-[11px] font-semibold text-muted">{formatAbilityMod(m[ab])}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="border-b border-rule-light pb-3 space-y-1">
          {m.savingThrows && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Saving Throws</span> {
          ...
          {m.skills && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Skills</span> {
          ...
          {m.damageResistances && m.damageResistances.length > 0 && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Damage Resistances</span> {m.damageResistances.join(', ')}</p>
          )}
          {m.damageImmunities && m.damageImmunities.length > 0 && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Damage Immunities</span> {m.damageImmunities.join(', ')}</p>
          )}
          {m.damageVulnerabilities && m.damageVulnerabilities.length > 0 && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Damage Vulnerabilities</span> {m.damageVulnerabilities.join(', ')}</p>
          )}
          {m.conditionImmunities && m.conditionImmunities.length > 0 && (
            <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Condition Immunities</span> {m.conditionImmunities.join(', ')}</p>
          )}
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Senses</span> {sensesStr}</p>
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Languages</span> {m.languages || '-'}</p>
          <p className="font-body text-[12px] font-semibold text-muted"><span className="font-bold text-ink">Challenge</span> {m.challengeRating} ({m.experiencePoints.toLocaleString()} XP)</p>
        </div>

        {m.traits && m.traits.length > 0 && (
          <div className="space-y-2">
            {m.traits.map((t, i) => (
              <div key={i} className="font-body text-[12px] font-semibold text-muted">
