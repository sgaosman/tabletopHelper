  ...
  return (
    <div className="min-h-screen bg-page">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <p className="font-body text-[13px] font-semibold text-muted mb-6">Search the complete spell list</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-3">
          ...
          <select value={concFilter} onChange={(e) => setConcFilter(e.target.value)}
            className="px-3 py-2 bg-card border border-rule font-body text-[14px] font-semibold text-ink focus:border-muted focus:outline-none">
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="font-body text-[12px] font-semibold text-muted">{totalElements} spells found</p>
          {hasFilters && (
                      ...
                      {spells.map(s => (
                        <tr key={s.id} onClick={() => setSelectedSpell(s)}
                          ...
                          <td className="px-3 py-2">
                            ...
                            {s.concentration && <span className="ml-2 font-heading text-[9px] font-semibold tracking-[0.02em] px-1.5 py-0.5 text-muted border border-rule">C</span>}
                            {s.ritual && <span className="ml-1 font-heading text-[9px] font-semibold tracking-[0.02em] px-1.5 py-0.5 text-cls-wizard border border-abj-border bg-abj-bg">R</span>}
                          </td>
                          <td className="px-3 py-2 font-body text-[13px] font-semibold text-muted">{levelLabel(s.level)}</td>
                          <td className="px-3 py-2 font-body text-[13px] font-semibold text-muted hidden md:table-cell">{s.school}</td>
                          <td className="px-3 py-2 font-body text-[13px] font-semibold text-muted hidden md:table-cell">{s.castingTime}</td>
                          <td className="px-3 py-2 font-body text-[13px] font-semibold text-muted hidden lg:table-cell">{s.rangeDistance}</td>
                          <td className="px-3 py-2 font-body text-[13px] font-semibold text-faint hidden lg:table-cell">{sourceName(s.source || '')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
